...
namespace varden {
	...
	const std::string funcName = "varden";
		...
		int				&efid)
	{
		...
		for (int i = 0; i < argc; ++i){
			...
			if ((arg == "-h") || (arg == "--help")){
				...
				if (i + 1 < argc){
					min = strtof(argv[i + 1].c_str(), NULL);
					i++;
				}
				else {
					std::cerr << "--input option requires one argument." << std::endl;
				...
				if (i + 1 < argc){
					max = strtof(argv[i + 1].c_str(), NULL);
					i++;
				}
				else {
					std::cerr << "--input option requires one argument." << std::endl;
				...
				if (i + 1 < argc){
					num = int(strtof(argv[i + 1].c_str(), NULL));
					i++;
				}
				else {
					std::cerr << "--input option requires one argument." << std::endl;
	...
	**************************************************************************************************/
	...
	int calculate(std::vector<std::string> argv){
		...
		int process_arg_status = process_arg(finBaseName, fout, argv, minBoxSize, maxBoxSize, numBoxSize, startFileId, endFileId);
