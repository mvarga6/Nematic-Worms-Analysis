...
namespace pofr {
	...
	const std::string funcName = "pofr";
		...
		int				&efid)
	{
		...
		for (int i = 1; i < argc; ++i){
			...
			if ((arg == "-h") || (arg == "--help")){
				...
				if (i + 1 < argc){
					width = strtof(argv[i + 1].c_str(), NULL);
					i++;
				}
				else {
					std::cerr << "--input option requires one argument." << std::endl;
				...
				if (i + 1 < argc){
					step = int(strtof(argv[i + 1].c_str(), NULL));
					i++;
				}
				else {
					std::cerr << "--input option requires one argument." << std::endl;
	...
	**************************************************************************************************/
	...
	int calculate(std::vector<std::string> argv){
		...
		int process_arg_status = process_arg(finBaseName, fout, argv, binWidth, dstep, startFileId, endFileId);
