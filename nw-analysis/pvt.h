	...
	int calculate(std::vector<std::string> argv){
		...
		for (int fid = startFileId; fid <= endFileId; fid++)
		{
			...
			while (!fin.eof())
			{
				...
				const float aveCos = cosSum / aveOver;
				orderParameter = aveCos;
				totalOrderParameter += orderParameter;

				std::cout << "P = " << orderParameter << std::endl;
				fout.flush();
			...
			std::cout << "P_ave = " << totalOrderParameter << std::endl;
